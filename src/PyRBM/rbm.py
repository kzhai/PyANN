...
class RestrictedBoltzmannMachine(network.Network):
    def __init__(self,
        ...
        self._input_shape = int(numpy.prod(lasagne.layers.get_output_shape(input_network)[1:]))
    ...
    def free_energy(self, input=None):
        if input == None:
            return self._neural_network.free_energy(self.get_input());
        else:
            return self._neural_network.free_energy(input);
    ...
    def get_cost_updates(self, learning_rate=0.1, persistent=None, k=1):
        ...
        if persistent is None:
            ...
            pre_sigmoid_ph, ph_mean, ph_sample = self._neural_network.sample_h_given_v(self.get_input())
        ...
        ([pre_sigmoid_nvs, nv_means, nv_samples, pre_sigmoid_nhs, nh_means, nh_samples], updates) = theano.scan(
            self._neural_network.gibbs_hvh, outputs_info=[None, None, None, None, None, chain_start], n_steps=k)
        # start-snippet-3
        # determine gradients on RBM parameters
        # note that we only need the sample at the end of the chain
        chain_end = nv_samples[-1]
        
        cost = T.mean(self.free_energy(self.get_input())) - T.mean(self.free_energy(chain_end))
    ...
    def get_pseudo_likelihood_cost(self, updates):
        ...
        cost = T.mean(self._input_shape * T.log(T.nnet.sigmoid(fe_xi_flip - fe_xi)))

        # increment bit_i_idx % number as part of updates
        updates[bit_i_idx] = (bit_i_idx + 1) % self._input_shape
    ...
    def get_reconstruction_cost(self, updates, pre_sigmoid_nv):
        ...
        cross_entropy = T.mean(T.sum(self.get_input() * T.log(T.nnet.sigmoid(pre_sigmoid_nv)) 
                                     + (1 - self.get_input()) * T.log(1 - T.nnet.sigmoid(pre_sigmoid_nv)), axis=1))
