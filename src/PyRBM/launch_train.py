...
def launch_train():
    ...
    import rbm
    network = rbm.RestrictedBoltzmannMachine(
            input_layer,
            layer_dimension)
    ...
    cost, sample, updates = network.get_cost_updates(learning_rate=learning_rate, persistent=persistent_chain, k=number_of_gibbs_steps)
    ...
    for epoch_index in range(number_of_epochs):
        ...
        for minibatch_index in xrange(number_of_minibatches):
            ...
            minibatch_cost, recovered_data = train_function(minibatch_x)
            minibatch_costs.append(minibatch_cost)

        clock_epoch = time.time() - clock_epoch;
        
        print 'epoch_index %i, average_cost %f, running time %fs' % (epoch_index, numpy.mean(minibatch_costs), clock_epoch)
